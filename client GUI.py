...
class MENU:
    ...
    def __init__(self):
        ...
        self.inputfield = tk.Entry(container, textvariable = self.ip, font=('Arial', 11), bg= 'white', border= 1, relief= 'solid', justify= 'center')
        self.inputfield.pack(pady= 15, ipadx=5, ipady=5)
        
        confirm = tk.Button(container, bg= ui_blue, fg='white', font=('Arial', 11), text='CONNECT', relief= 'flat', borderwidth= 0, activebackground='#2980b9', activeforeground= 'white', command=self.redirect)
    ...
    def redirect(self):
        ...
        address = self.inputfield.get()
        self.root.destroy()
        self.chat = CHAT(address)
        self.chat.begin()
...
class CHAT:
    ...
    def begin(self):
        ...
        print(f'Connected to server at {self.address}:{server_port}')
        ...
        """ while True:
            ...
            else: self.socket.send(message.encode())
